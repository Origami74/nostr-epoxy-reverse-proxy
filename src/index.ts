...
async function putInWallet(tokens: Proof[], mintUrl: string) {
  ...
  if(nutSackAmount >= profitsPayoutThreshold){
    ...
    if(profitPubkeyLockEnabled){
      redeemedTokens = await wallet.receiveTokenEntry({proofs: nutSack, mint: mintUrl}, {privkey: relayPrivateKey, pubkey: `02${profitsPubkey}`});
    } else {
    ...
    const encryptedDmContent = await new NSecSigner(relayPrivateKey).nip04.encrypt(profitsPubkey, `Here's your profits from your relay proxying service. At ${new Date().toUTCString()}.\n ${encodedCashuToken}`)
...
async function handleCustomerMessage(customerSocket: WebSocket, message: MessageEvent, useBuffer: boolean, buffer: MessageEvent[]): Promise<void> {
  try {
